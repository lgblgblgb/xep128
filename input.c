/* Human Input Devices emulation,
 * ie keyboard, mouse, joystick */
...
static const int keytable[][3]={
	...
	{ SDL_SCANCODE_CAPSLOCK,1,0x02},/*TODObacktickascaps-becausefirefoxscrewsupaloadofkeycodeswhenpressingshift*/
	...
	{ SDL_SCANCODE_LCTRL,1,0x80},/*[CTRL]TODOsym-gah,firefoxscrewsupctrl+keytoo*/
	{ SDL_SCANCODE_SPACE,8,0x40},/*space*/
	{ SDL_SCANCODE_SEMICOLON,6,0x08},/*;hmmonchromeIgetthiskey*/
	{ SDL_SCANCODE_LEFTBRACKET,9,0x20},/*[*/
	{ SDL_SCANCODE_RIGHTBRACKET,6,0x40},/*]*/
	{ SDL_SCANCODE_APOSTROPHE,6,0x20},/*'forreal,butwemapEP:here*/
	...
	{ SDL_SCANCODE_EQUALS,9,0x08},/*=,butwemapas@*/
	...
	{ SDL_SCANCODE_F9,7,0x80},/*F9,mappedas"ALT"*/
	{ SDL_SCANCODE_HOME,7,0x10},/*home,wemapthisasHOLD*/
	{ SDL_SCANCODE_END,7,0x01},/*end,wemapthisasSTOP*/
	/* Not real EP kbd matrix, used for extjoy emulation with numeric keypad */
	{ SDL_SCANCODE_KP_5,10,0x01},//FIRE(numkeypad5)
	{ SDL_SCANCODE_KP_8,10,0x02},//UP(numkeypad8)
	{ SDL_SCANCODE_KP_2,10,0x04},//DOWN(numkeypad2)
	{ SDL_SCANCODE_KP_4,10,0x08},//LEFT(numkeypad4)
	{ SDL_SCANCODE_KP_6,10,0x10},//RIGHT(numkeypad6)
	{ -1, -1, -1 }
};
...
{
	fprintf(stderr, "MOUSE BUTTON %d press = %d\n", button, press);
...
void emu_mouse_motion(int dx, int dy)
{
	...
	fprintf(stderr, "MOUSE MOTION event dx = %d dy = %d\n", dx, dy);
	_mouse_dx -= dx;
...
void emu_kbd(SDL_Keysym sym, int press)
{
	...
	fprintf(stderr, "KEY: scan=%d sym=%d press=%d\n", sym.scancode, sym.sym, press);
	while (keytable[a][0] != -1)
		if (keytable[a][0] == sym.scancode) {
			...
			fprintf(stderr, "  to EP key %dd mask=%02Xh\n", keytable[a][1], keytable[a][2]);
			return;
		} else
			a++;
}
